...
export async function GET(
  ...
  try {
    ...
    const person = await People.findById(params.id);
    if (!person) {
    ...
    // Get additional data
    const progress = await Progress.findByPersonId(params.id);
    const attendance = await Attendance.findByPersonId(params.id);
...
export async function PATCH(
  ...
  try {
    ...
    const existing = await People.findById(params.id);
    if (!existing) {
      return errors.notFound('Person');
    }
    ...
    const person = await People.update(params.id, body);
