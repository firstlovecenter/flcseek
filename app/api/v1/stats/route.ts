...
export async function GET(request: NextRequest) {
  try {
    ...
    const peopleCountResult = await queryOne<{ count: number }>(`
      ...
      ${filters.year ? 'LEFT JOIN groups g ON nc.group_id = g.id' : ''}
      ${whereClause}
    `, values);
    
    // Get groups count for current year
    ...
    if (user!.role === ROLES.SUPERADMIN || user!.role === ROLES.LEADPASTOR) {
      const groupStatsResult = await query<{
        ...
        LEFT JOIN progress_records pr ON nc.id = pr.convert_id
        WHERE g.year = $2 AND g.is_active = true
        GROUP BY g.id, g.name
        ORDER BY g.name
      `, [milestonesCountResult?.count || 1, currentYear]);
