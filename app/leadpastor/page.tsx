...
export default function LeadPastorDashboard() {
  ...
  Object.keys(groupsByYear).forEach(year => {
    groupsByYear[parseInt(year)].sort((a, b) => {
  ...
  const handleGroupClick = (groupName: string, year: number) => {
    router.push(`/leadpastor/${groupName.toLowerCase()}?year=${year}`);
  };
                ...
                <Row gutter={[24, 24]} style={{ paddingTop: 8 }}>
                  {groupsByYear[year].map((group, index) => (
                    <Col xs={24} sm={12} md={8} lg={6} key={group.id}>
                      <Card
                        hoverable
                        onClick={() => handleGroupClick(group.name, group.year)}
                        style={{
                          borderRadius: 12,
