...
export async function loadHierarchy(): Promise<Hierarchy> {
  const [ccfs, ccgs, councils] = await Promise.all([
...
export async function loadScope(userId: string): Promise<CcgScope> {
  const [user, assignments, hierarchy] = await Promise.all([
  ...
  return resolveCcgScope(grants, hierarchy)
}
